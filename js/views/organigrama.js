...
import { kpisByPersona, avgAchievement, ratingFromScore } from '../calc.js';
...
export function renderOrganigrama() {
  ...
  const validIds = new Set(appData.personas.map((p) => p.id));
  const roots = appData.personas.filter((p) => !p.reportsTo || !validIds.has(p.reportsTo));
...
function orgNodeHtml(id, visited) {
  ...
  const children = appData.personas.filter((x) => x.reportsTo === id);
  return `<li>
    <div class="org-card" data-orgnode="${id}">
      <div class="avatar" style="background:${colorFor(id, appData.personas)};margin:0 auto;">${initials(p.name)}</div>
